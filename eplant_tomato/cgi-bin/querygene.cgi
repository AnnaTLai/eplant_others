...
try:
    ...
    if species == 'Solanum_lycopersicum':
        con = MySQLdb.connect('localhost', 'hans', 'un1pr0t', 'eplant_tomato')
        cur = con.cursor()
        terms = term.split('/')
        # This doesn't work if AGI an annotation is entered!
        #if len(terms) > 1:
        ...
        if result:
            id = result[0]
            query = 'SELECT geneId,start,end,strand,seqId FROM gff3 WHERE (type=\"gene\" or type=\"transposable_element_gene\") AND geneId=\"' + id + '\" AND Source=\"phytozomev11\";'
            cur.execute(query)
            data = cur.fetchone()
            gene['id'] = data[0]
