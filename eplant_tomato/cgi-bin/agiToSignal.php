...
function getVerifiedAGI($type, $data) {
	...
	# Regex match
	if (preg_match("/^(Solyc\d+[M|C|G]\d+$)/i", $data, $match)) {
		$data = $match[1];
	} else {
		$error[$type] = $data;
...
function getVerifiedDataSource($data) {
	...
	array_push($datasources, 'soybean');
...
function getSignal($primary_agi, $secondary_agi, $mode, $data_source, $sample) {
	...
	$date = mysqli_real_escape_string($con, "2014-10-29");
	...
	# For absolute mode
	if ($mode == "Absolute") {
		if ($data_source == "tomato" || $data_source == "tomato_renormalized") {
			if ($sample == "epidermis" || $sample == "TOMATO_CTRL_454" || $sample == "collenchyma" || $sample == "vascular" || $sample == "parenchyma" || $sample == "endodermis") {
				# 454
				$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT t2.454_id FROM tomato_annotations_lookup.454_Illumina_lookup t2 WHERE t2.Illumina_id='$primary_agi' AND t2.date='2012-09-25' LIMIT 1)";
			} else {
				# Illumina Data
		...
		} elseif ($data_source == "tomato_ils" || $data_source == "tomato_ils2" || $data_source == "tomato_s_pennellii"){
				$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id like '$primary_agi%'";
		} else {
			$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT t2.probeset FROM soybean_annotations_lookup.gene_probeset_lookup t2 WHERE t2.gene='$primary_agi' AND t2.date='$date' LIMIT 1)";
		}
	}
