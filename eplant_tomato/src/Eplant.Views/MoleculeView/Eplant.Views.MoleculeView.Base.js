	...
	Eplant.Views.MoleculeView.Base = function(customConfig, customInfo, viewer) {
		...
		this.config = {
			...
			*/
			application : {
				...
				cddUrl : null,
				//
				pfamUrlBar : '//bar.utoronto.ca/eplant/cgi-bin/PfamAnnot.cgi',
				//
				pfamUrlDev : 'ProxyServlet',
				//
				cddUrlBar : '//bar.utoronto.ca/eplant/cgi-bin/CDDannot.cgi',
				//
		...
		*/
		this.info = {
			...
			serverURL : "//bar.utoronto.ca/eplant/cgi-bin/jsmol.php",
			src : null,
					...
					for (var i=this.i, innerlen=i+2; i<innerlen&&i<res.length; i++) {
						...
						if(lastMatchNumber&&matchNumber!=lastMatchNumber+1){
							...
							for (var j=lastMatchNumber, jlen=matchNumber-1; j<jlen; j++) {
								...
								// Asher 2019: This fixes a grave bug (undefined in sequence!)
			...
			for (var i=0, len=res.length; i<len; i+=2) {
				Eplant.queue.add(function(){
					for (var i=this.i, innerlen=i+2; i<innerlen&&i<res.length; i++) {
						...
						// Asher 2019: This part is most likely not used
						//keep only aminoacids dropping everything else, like HOH
						if (matches && codeArray[matches[1]]) {
							if (codeArray.hasOwnProperty(matches[1])) {
								this.viewer.mySequence.seq1letterArr.push(codeArray[matches[1]]);
								this.viewer.mySequence.seq3letterArr.push(matches[1]);
								this.viewer.mySequence.fullseq3letterArr.push(res[i]);
								this.viewer.mySequence[res[i]]={oneLetterCode:codeArray[matches[1]],threeLetterCode:matches[1],selected:false};
			...
			success: $.proxy(function(data, textStatus, jqXHR ){
				...
				if(Object.keys(data).length==0){//empty
					...
					$.each(data, $.proxy(function(key, value) {
						...
						// Asher 2019: Pfam bug fix

						var start = this.mySequence.model3letterArr[value.startIndex - 1];
						var end = this.mySequence.model3letterArr[value.endIndex - 1];
						...
						$(this.pDiv+this.jq(this.config.CSS.IDs.PfamDomains)).append("<span id='"+this.config.CSS.IDs.parentDivId+key+"span' class='radioMenu'><input type='radio' id='"+this.config.CSS.IDs.parentDivId+key +"' name='"+this.config.CSS.names.rdFeature+"' value='TODO' style=''><label for='"+this.config.CSS.IDs.parentDivId+key +"'>" + value.PfamAnnot+": " + start +" - " + end +" ("+value.domainName+")</label></span>");
	...
	*/
	Eplant.Views.MoleculeView.Base.prototype.ajaxGetSNPfeatures= function(agi){
		...
		this.viewer.Xhrs.getSNPXhr=$.ajax({
			type: 'GET',
			dataType: 'json',
			url: '//bar.utoronto.ca/eplant/cgi-bin/get_snp_data.php',
			timeout: 25000,
