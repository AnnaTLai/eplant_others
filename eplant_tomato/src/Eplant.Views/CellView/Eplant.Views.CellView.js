	...
	*/
	...
	Eplant.Views.CellView.prototype.generateCitation = function() {
		this.citation='<h2>Citation and Experiment Information for this View</h2><br>';
			this.citation += "The data come from Hooper CM, Castleden IR, Aryamanesh N, Jacoby RP, Millar AH. (2017). Finding the Subcellular Location of Barley, Wheat, Rice and Maize Proteins: The Compendium of Crop Proteins with Annotated Locations (cropPAL). Plant Cell Physiol. 2016 Jan;57(1):e9<br><br> We apply the formula indicated in the Material and Methods section of Winter et al. 2007 (doi: 10.1371/journal.pone.0000718) to generate a confidence score for each distinct subcellular compartment or region, with experimentally-determined localizations being weighted five times more than predicted ones. The higher the confidence score for a given subcellular compartment, the more intense the red colour in the Cell eFP Browser output. <br><br> See further details at CropPAL: <a target='_blank' href='http://crop-pal.org/'>http://crop-pal.org/</a>";
	...
	Eplant.Views.CellView.prototype.loadData = function() {
		...
		this.Xhrs.loadDataXhr = $.ajax({
			...
			success: $.proxy(function(response) {
				...
				this.webService = Eplant.ServiceUrl + "groupsuba4.php";
				...
				for (var n = 0; n < this.samples.length; n++) {
					if ($.inArray(this.samples[n].name, sampleNames) === -1) {
						sampleNames.push(this.samples[n].name);
					}
				}
			},this)
		});
		
		/* JS promises used to avoid ajax call going before samples are loaded. */
		this.Xhrs.loadDataXhr.done($.proxy(function() {
				...
				/* Now call the SUBA4 webservice on the bar */
				$.ajax({
					url: this.webService,
					type: 'POST',
					contentType: 'application/json',
					dataType: 'json',
					data: JSON.stringify({AGI_IDs: this.geneticElement.identifier.split(" "), include_predicted: true}),
					success: $.proxy(function(response) {
						if(response[0].includes_predicted === "yes"){
						...
						/* New Code for SUBA4 data */
						for (var n = 0; n < this.samples.length; n++) {
							$.each(response[0].data, $.proxy(function(loc, value) {
								if (this.samples[n].name.toUpperCase() == loc.toUpperCase()) {
									this.samples[n].value = value;
									return false;
								}
							}, this));
						}
