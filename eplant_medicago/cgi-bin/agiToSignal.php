...
function getVerifiedAGI($type, $data) {
	...
	# Regex match
	if (preg_match("/^(Medtr\d+g\d+)$/i", $data, $match)) {
		$data = $match[1];
	} else {
		$error[$type] = $data;
...
function getSignal($primary_agi, $secondary_agi, $mode, $data_source, $sample) {
	...
	#$date = mysqli_real_escape_string($con, "2010-05-27");
	...
	# For absolute mode
	if ($mode == "Absolute") {
		if ($data_source == 'medicago_seed') {
			$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT probeset from medicago_annotations_lookup.medicago_seed_lookup where mtgi='$primary_agi' LIMIT 1)";
		} else {
			$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT probeset from medicago_annotations_lookup.at_mtgi_lookup_merged where mtgi='$primary_agi' LIMIT 1)";
		}
	}
