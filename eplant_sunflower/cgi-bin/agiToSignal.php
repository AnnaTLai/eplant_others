...
# Author: Asher
# Date: May, 2014
# Usage: http://bar.utoronto.ca/eplant_sunflower/cgi-bin/agiToSignal.php?dataSource=sunflower&primaryGene=HanXRQChr01g0000151&sample=Leaves
################################################################################
...
function getVerifiedAGI($type, $data) {
	...
	# Regex match
	if (preg_match("/^(HanXRQChr\d+[M|C|G]\d*g*\d+$)/i", $data, $match)) {
		$data = $match[1];
	} else {
		$error[$type] = $data;
...
function getVerifiedDataSource($data) {
	...
	array_push($datasources, 'sunflower');
...
function getSignal($primary_agi, $secondary_agi, $mode, $data_source, $sample) {
	...
	# For absolute mode
	if ($mode == "Absolute") {
		$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT t2.ha412 FROM eplant_sunflower.gene_mapping t2 WHERE t2.haxrq='$primary_agi' LIMIT 1)";
	}
