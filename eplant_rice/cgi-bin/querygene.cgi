...
try:
    ...
    if species == 'Oryza_sativa':
        con = MySQLdb.connect('localhost', 'hans', 'un1pr0t', 'eplant_rice')
        cur = con.cursor()
        terms = term.split('/')
        # This doesn't work if AGI an annotation is entered!
        #if len(terms) > 1:
        ...
        #cur.execute(query)
        #result = cur.fetchone()
        query = 'SELECT geneId FROM gff3_msu WHERE (type=\"gene\" OR type=\"transposable_element_gene\" or type=\"protein_coding_gene\") AND geneId LIKE "%' + term + '%" LIMIT 1;'
        cur.execute(query)
        result = cur.fetchone()
        if result:
            id = result[0]
            query = 'SELECT geneId,start,end,strand,seqId FROM gff3_msu WHERE (type=\"gene\" or type=\"transposable_element_gene\" or type=\"protein_coding_gene\") AND geneId=\"' + id + '\" AND Source=\"ensembl\";'
            cur.execute(query)
            data = cur.fetchone()
            gene['id'] = data[0]
            ...
            #    aliases.append(row[0])
            #gene['aliases'] = aliases
            query = 'SELECT annotation FROM gene_annotation WHERE gene=\"' + term + ".1" + '\";'
            cur.execute(query)
            gene['annotation'] = None
            for row in cur:
                temp = row[0].split('__')
