...
function getVerifiedAGI($type, $data) {
	...
	# Regex match
	if (preg_match("/^(LOC_Os\d+[M|C|G]\d+$)/i", $data, $match)) {
		$data = $match[1];
	} else {
		$error[$type] = $data;
...
function getVerifiedDataSource($data) {
	...
	array_push($datasources, 'rice_leaf_gradient');
...
function getSignal($primary_agi, $secondary_agi, $mode, $data_source, $sample) {
	...
	$date = mysqli_real_escape_string($con, "2009-11-15");
	...
	# For absolute mode
	if ($mode == "Absolute") {
		if ($data_source == 'rice_leaf_gradient') {
			$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id='$primary_agi'";	
		} else {
			$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT t2.probeset FROM rice_annotations_lookup.at_loc_lookup t2 WHERE t2.loc='$primary_agi' AND t2.date='$date' LIMIT 1)";
		}
	}
