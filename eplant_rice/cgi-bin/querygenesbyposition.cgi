...
try:
    ...
    cur1 = con.cursor()
    cur2 = con.cursor()
    query1 = 'SELECT geneId,start,end,strand FROM gff3_msu WHERE type="protein_coding_gene" AND SeqID = "' + chromosome  + '" AND ((start>=' + start + ' AND start<=' + end + ') OR (end>=' + start + ' AND end<=' + end + ') OR (start<' + start + ' AND end>' + end + '));'
    cur1.execute(query1)
    genes = []
    for row1 in cur1:
        gene = {}
