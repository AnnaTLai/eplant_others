...
class Interactions
{
	...
	private function execute($mysqli) {
		...
		for ($i = 0; $i < count($this->JSONData); $i++) {
			if (isset($this->JSONData[$i]["agi"])) {
				...
				// Run the first query
				if ($interactors = $mysqli->query("SELECT Protein1, Protein2, Quality, Pcc, Bind_id, `Index` FROM interactions WHERE Protein1 = '$gene'")) {
					...
					if ($interactors->num_rows < 50) {
						...
						while ($row = $interactors->fetch_assoc()) {
							...
							if (is_null($row["Bind_id"])) {
								...
								if ($row["Index"] == "2" && $row["Bind_id"] != "PubMed22037706") {
									$published = false;
								} else {
									$published = true;
								}
							...
							if ($row["Index"] == "2" && $row["Bind_id"] == "PubMed25352272") {
								$published = true;
							}
				...
				if ($getRecursive) {
					...
					$query = "SELECT Protein1, Protein2, Quality, Pcc, Bind_id, `Index` FROM interactions WHERE (Protein1='" . $gene . "' AND Protein2='" . $gene . "')";
				...
				if (isset($data)) {	
					...
					$this->end("Data is not set");
				}
			} else {
				$this->end("Error adding data.");
			}
		}
		return $finalData;
	}
	...
	private function cleanInput($data) {
		...
		if (preg_match("/^LOC_OS\d+G\d+$/i", $data)) {
			return $data;	
		} else {
			$this->end("Invalid gene ID.");
		}
	}

	// Connect to database
	private function connect() {
		$mysqli = new mysqli("localhost", "InteractionsUser", "InteractionsUser", "rice_interactions");
		
		if ($mysqli->connect_errno) {
			$this->end("Can not connect to database");
		}
		return $mysqli;
	}
	...
	// Exit with error
	private function end($message = "An error has Occured") {
