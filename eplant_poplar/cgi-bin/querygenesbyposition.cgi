...
try:
    ...
    cur1 = con.cursor()
    cur2 = con.cursor()
    query1 = 'SELECT geneId,start,end,strand FROM gff3_v3 WHERE type="gene" AND SeqID = "' + chromosome  + '" AND ((start>=' + start + ' AND start<=' + end + ') OR (end>=' + start + ' AND end<=' + end + ') OR (start<' + start + ' AND end>' + end + '));'
    cur1.execute(query1)
    genes = []
    for row1 in cur1:
        gene = {}
        ...
        # Get annotation
        query2 = 'SELECT annotation FROM gene_annotation WHERE gene="' + gene['id'] + '.1";'
        cur2.execute(query2)
