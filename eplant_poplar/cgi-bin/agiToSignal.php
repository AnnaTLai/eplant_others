...
function getVerifiedAGI($type, $data) {
	...
	# Regex match
	if (preg_match("/^(Potri\.\d+[M|C|G]\d+$)/i", $data, $match)) {
		$data = $match[1];
	} else {
		$error[$type] = $data;
...
function getVerifiedDataSource($data) {
	...
	array_push($datasources, 'poplar_leaf');
	array_push($datasources, 'poplar_xylem');
...
function getVariables() {
	...
	if ($data_source == "poplar_leaf" || $data_source == "poplar_xylem") {
		// Don't add .1
	} else {
		$primary_agi .= ".1";	# Append this for Poplar ePlant
	}
...
function getSignal($primary_agi, $secondary_agi, $mode, $data_source, $sample) {
	...
	$date = mysqli_real_escape_string($con, "2014-09-22");
	...
	if ($mode == "Absolute") {
		if ($data_source == "poplar_leaf" || $data_source == "poplar_xylem") {
			...
			$query = "SELECT data_signal FROM sample_data WHERE data_bot_id='$sample' AND data_probeset_id=(SELECT t2.probeset FROM poplar_new_annotations.at_pgi_lookup t2 WHERE t2.pgi='$primary_agi' AND t2.date='$date' LIMIT 1)";
		}
	}
