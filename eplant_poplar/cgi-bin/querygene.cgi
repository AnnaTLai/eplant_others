...
try:
    ...
    if species == 'Populus_trichocarpa':
        con = MySQLdb.connect('localhost', 'hans', 'un1pr0t', 'eplant_poplar')
        cur = con.cursor()
        terms = term.split('/')
        # This doesn't work if AGI an annotation is entered!
        #if len(terms) > 1:
        ...
        #cur.execute(query)
        #result = cur.fetchone()
        query = 'SELECT geneId FROM gff3_v3 WHERE (type=\"gene\" OR type=\"transposable_element_gene\") AND geneId LIKE "%' + term + '%" LIMIT 1;'
        cur.execute(query)
        result = cur.fetchone()
        if result:
            id = result[0]
            query = 'SELECT geneId,start,end,strand,seqId FROM gff3_v3 WHERE (type=\"gene\" or type=\"transposable_element_gene\") AND geneId=\"' + id + '\" AND Source=\"phytozomev10\";'
            cur.execute(query)
            data = cur.fetchone()
            gene['id'] = data[0]
