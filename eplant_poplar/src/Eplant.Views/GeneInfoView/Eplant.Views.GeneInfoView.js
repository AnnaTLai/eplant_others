(function() {
	...
	*/
	Eplant.Views.GeneInfoView = function(geneticElement) {
		...
		$(this.domContainer).attr('id','app/geneInfo'+this.geneticElement.identifier.replace('.',''));
	...
	*/
	...
	Eplant.Views.GeneInfoView.prototype.loadData = function() {
		$.when(Eplant.Views.GeneInfoView.Params.htmlPage).then($.proxy(function( res ) {
			...
			$(clone).find('#info_locus').attr('id', "info_locus" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_aliases').attr('id', "info_aliases" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_fullName').attr('id', "info_fullName" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_briefDescription').attr('id', "info_briefDescription" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_computationalDescription').attr('id', "info_computationalDescription" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_curatorSummary').attr('id', "info_curatorSummary" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_location').attr('id', "info_location" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_holder').attr('id', "info_holder" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_chromosome_start').attr('id', "info_chromosome_start" + this.geneticElement.identifier.replace('.',''));
			$(clone).find("#info_chromosome_end").attr('id', "info_chromosome_end" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_strand').attr('id', "info_strand" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#publicationsTable').attr('id', "publicationsTable" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#geneRIFsTable').attr('id', "geneRIFsTable" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#geneModel').attr('id', "geneModel" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_DNAsequence').attr('id', "info_DNAsequence" + this.geneticElement.identifier.replace('.',''));
			$(clone).find('#info_proteinSequence').attr('id', "info_proteinSequence" + this.geneticElement.identifier.replace('.',''));


			var config = {
				IDs: {
					divLocusId: 'info_locus' + this.geneticElement.identifier.replace('.',''),
					divAliasesId: 'info_aliases' + this.geneticElement.identifier.replace('.',''),
					divFullNameId: 'info_fullName' + this.geneticElement.identifier.replace('.',''),
					divBriefDescriptionId: 'info_briefDescription' + this.geneticElement.identifier.replace('.',''),
					divComputationalDescriptionId: 'info_computationalDescription' + this.geneticElement.identifier.replace('.',''),
					divCuratorSummaryId:'info_curatorSummary'+ this.geneticElement.identifier.replace('.',''),
					divInfoHolderId:'info_holder'+ this.geneticElement.identifier.replace('.',''),
					divLocationId:'info_location'+ this.geneticElement.identifier.replace('.',''),
					divChromosomeStartId:"info_chromosome_start" + this.geneticElement.identifier.replace('.',''),
					divChromosomeEndId:'info_chromosome_end' + this.geneticElement.identifier.replace('.',''),
					divStrandId:'info_strand' + this.geneticElement.identifier.replace('.',''),
					divPublicationsTableId:'publicationsTable'+ this.geneticElement.identifier.replace('.',''),
					divGeneRIFsTableId:"geneRIFsTable" + this.geneticElement.identifier.replace('.',''),
					divGeneModelId:'geneModel' + this.geneticElement.identifier.replace('.',''),
					divDNAsequenceId:'info_DNAsequence' + this.geneticElement.identifier.replace('.',''),
					divProteinSequenceId:'info_proteinSequence' + this.geneticElement.identifier.replace('.','')
				}
			...
			var getGeneSummary = $.ajax({
				type: 'GET',
				url: '//bar.utoronto.ca/~asher/eplant_poplar/cgi-bin/gene_summary_by_locus.php?locus=' + this.geneticElement.identifier,
				dataType: 'json',
			...
			var getGeneStructure = $.ajax({
				type: "GET",
				dataType: "json",
				url: '//bar.utoronto.ca/~asher/eplant_poplar/cgi-bin/gene_structure_by_locus.php?locus='+this.geneticElement.identifier,
				success: $.proxy(function(data) {
					...
					if (this.geneticElementType === 'protein_coding') {
						$.ajax({
							type: "GET",
							dataType: "json",
							url: '//bar.utoronto.ca/~asher/eplant_poplar/cgi-bin/get_protein_sequence_by_identifier.php?locus='+this.geneticElement.identifier,
					...
					var getGenomeSequence = $.ajax({
						type: "GET",
						dataType: "json",
						url: '//bar.utoronto.ca/~asher/eplant_poplar/cgi-bin/get_sequence_by_identifier.php?locus=' + this.geneticElement.identifier,
						success: $.proxy(function(data) {
