(function() {
	...
	*/
    ...
    //BEN CHANGED: the following is the parent method copy and pasted but with the removal of the domContainer removed
	...
	Eplant.BaseViews.EFPView.prototype.generateScreenshotSvgStr = function(citation) {
		...
		// Asher fixing errors in SVG!
		var finalSvg = $(returnedSvg)[0].outerHTML.replace("display: none;","");
		finalSvg = finalSvg.replace("style=\"display: none;\"","");
		finalSvg = finalSvg.replace("etc.\"", "etc.\"/");
		return finalSvg;
	};
			...
			success: $.proxy(function(response) {
				...
				if (webServiceXml.length > 0) {
					...
					if(this.database){
						...
						this.webService = Eplant.ServiceUrl + "plantefp.cgi?datasource=poplar&";
					}
				}
