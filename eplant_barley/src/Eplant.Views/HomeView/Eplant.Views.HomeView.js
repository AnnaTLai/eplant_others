(function() {
	...
	*/
	Eplant.Views.HomeView = function() {
		...
		/**
     * Fires the loadfinished event for the home view 
     * Is an event listener because we need it to fire after the species is loaded
     */
    var finishedLoadingEventListener =  new ZUI.EventListener('update-activeSpecies', null, (event, eventData, listenerData) => {
      console.log("Home View event listern fired")
      this.loadFinish();
    });

    console.log('Home View listener added')
    ZUI.addEventListener(finishedLoadingEventListener);
