...
* @MySQL	Matt Ierullo
* @Date		July, 2014
* @Usage	http://bar.utoronto.ca/eplant/cgi-bin/new_get_interactions.php?request=[{"agi":"At1g02130"},{"agi":%"At4g23810"},{"agi":"At3g01090"}]	
*/
	...
	private function execute($mysqli) {
		...
		for ($i = 0; $i < count($this->JSONData); $i++) {
			if (isset($this->JSONData[$i]["agi"])) {
				...
				// Run the first query
				if ($interactors = $mysqli->query("SELECT Protein1, Protein2, Quality, Pcc, Bind_id, aiv_index FROM interactions_eplant WHERE Protein1 = '$gene'")) {
				...
				if ($getRecursive) {
					...
					$query = "SELECT Protein1, Protein2, Quality, Pcc, Bind_id, aiv_index FROM interactions_eplant WHERE (Protein1='" . $gene . "' AND Protein2='" . $gene . "')";
