(function () {
	...
	*/
	...
	Eplant.Views.InteractionView.LoadInteraction.prototype.createChromosomes = function (data) {
		...
		// Start with default experimental, then switch to predited if found
		var methods = { '1': 'P', '2': 'P', '3': 'P', '4': 'P', '5': 'P' };
		...
		for (var c in chrNums) {
			...
			this.createChromosomeEdge(chrs[i], methods[c]);
			i++;
		}
	};
	 ...
	 * @return {void}
	 */
	Eplant.Views.InteractionView.LoadInteraction.prototype.createChromosomeEdge = function (id, method) {
		// Asher
		var source = this.geneticElement.identifier.toUpperCase();
		source = source === this.query ? source + 'QUERY_NODE' : source + 'PROTEIN_NODE';
		var edge = {
			data: {
				source: source,
	...
	function loadJSON (query, resolve) {
		var u = '//bar.utoronto.ca/eplant_maize/cgi-bin/get_interactions.py';
		var req = '?locus=' + query;
		$.getJSON(u + req, function (response) {
			resolve(response);
		});
	}
}());
