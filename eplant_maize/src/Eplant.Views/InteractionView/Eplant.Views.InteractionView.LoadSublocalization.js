(function () {
	...
	function loadData (resolve) {
		...
		var urlSUBA = '//bar.utoronto.ca/eplant/cgi-bin/groupsuba4.php';
		...
		$.ajax({
			...
			dataType: 'json',
			data: JSON.stringify({AGI_IDs: ids, include_predicted: false}), 
			success: $.proxy(function(response) {
				// $.getJSON(expQuery, function (response) {
				...
				$.ajax({
					...
					contentType: 'application/json',
					dataType: 'json',
					data: JSON.stringify({AGI_IDs: predIds, include_predicted: true}),
					success: $.proxy(function (response) {
