...
Author: Asher
Date: January, 2018
Usage: python3 getLargeData.py
Example: http://www.bar.utoronto.ca/eplant/cgi-bin/get_interactions_dapseq.py?locus=AT3G24650
"""
import MySQLdb
import cgi
import re
...
def connect():
    ...
    try:
        conn = MySQLdb.connect(host='localhost', user='InteractionsUser', passwd='InteractionsUser', db='maize_interactions')
...
def second_query(conn, agis):
    ...
    # Query PPIs
    cursor = conn.cursor()
    sql = 'SELECT Protein1, Protein2, aiv_index, Quality, Pcc, Bind_id FROM interactions WHERE aiv_index < 2 AND Protein1 IN ("' + '", "'.join(
        agis.keys()) + '")'
...
def add_PDIs(conn, agi, data):
    ...
    # Query PPIs
    cursor = conn.cursor()
    sql = 'SELECT Protein1, Protein2, aiv_index, Quality, Pcc, Bind_id  FROM interactions WHERE aiv_index = 2 AND Protein1 = %s'
...
def output_data(recursive, agi, data):
    ...
    for row in data:
        ...
        if row[4] is None:
            ...
            output['correlation_coefficient'] = float(row[4])
...
def is_locus_valid(locus):
    ...
    """
    if re.match(r"^GRMZM\d*.G\d+$", locus, re.IGNORECASE):
        return True
    else:
        return False
